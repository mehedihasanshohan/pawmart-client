...
const Navbar = () => {
  ...
  // 1. LocalStorage theke theme check kora
  const [theme, setTheme] = useState(localStorage.getItem("theme") || "light");

  // 2. Theme change hole HTML tag-e attribute update kora
  useEffect(() => {
  ...
  return (
    <div className="navbar sticky z-50 backdrop-blur-md top-0 bg-base-100 shadow-sm transition-all duration-300">
      <div className="navbar-start">
        {/* Dropdown for Mobile (Keep your existing code) */}
        <div className="dropdown">
          <div tabIndex={0} role="button" className="btn lg:hidden">
          ...
          <ul tabIndex="-1" className="menu menu-md dropdown-content bg-base-200 rounded-box z-1 mt-3 w-52 p-2 shadow">
            <li><NavLink to='/'>Home</NavLink></li>
            <li><NavLink to='/listings'>Pets & Supplies</NavLink></li>
            {user && (
              <>
      ...
      <div className="navbar-end flex items-center gap-3">
        ...
        >
          {theme === "light" ? <FaRegMoon /> : <PiSunBold className="text-yellow-400" />}
        </button>
        ...
        {user ? (
          ...
          <div className='flex gap-2'>
            <NavLink to="/login" className="btn btn-sm btn-accent">Login</NavLink>
            <NavLink to="/register" className="btn btn-sm btn-info">Register</NavLink>
          </div>
        )}
      </div>
    </div>
