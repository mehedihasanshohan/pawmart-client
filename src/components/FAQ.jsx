...
export default function FAQ() {
  ...
  return (
    <section className="py-20 bg-base-200 text-base-content overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-3xl font-bold mb-8 text-center">Common Questions</h2>
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="border border-slate-500 rounded-md overflow-hidden bg-base-100">
              <button
              ...
              <div className={`transition-all duration-300 ease-in-out ${openIndex === i ? 'max-h-40 p-6 pt-0 opacity-100' : 'max-h-0 opacity-0'}`}>
                <p className="opacity-80 text-sm italic border-t border-slate-700 pt-4">{faq.a}</p>
              </div>
            </div>
          ))}
        </div>
